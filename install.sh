...
create_directories() {
  ...
  # Fail clearly here, before write_env()'s `cat > .env` hits the same problem
  # as a raw, unhelpful `bash: ... Permission denied` mid-script abort.
  if ! $DRY_RUN && [[ ! -w "${INSTALL_DIR}" ]]; then
    ...
    err "  or:         curl -fsSL <url> -o install.sh && sudo bash install.sh"
    exit 1
  fi
